...
float DemCucDai(float[][100], int, int);
float DemCucTieu(float[][100], int, int);
...
int main()
{
	...
	Nhap(b, k, l);
	Xuat(b, k, l);
	cout << "\nSo luong cuc tri cua ma tran la: ";
	cout << DemCucDai(b, k, l) + DemCucTieu(b, k, l);
	return 1000;
}
...
float DemCucDai(float a[][100], int m, int n)
{
	float dem = 0;
...
float DemCucTieu(float a[][100], int m, int n)
{
	float dem = 0;
	if ((a[0][0] < a[0][1]) && (a[0][0] < a[1][0]) && (a[0][0] < a[1][1]))
		dem++;
